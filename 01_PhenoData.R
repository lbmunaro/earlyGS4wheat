...
ILYT_Pheno <- ILYT_Pheno_w |>
  ...
  mutate(Stage=as.factor('YT')) |>
  # Select relevant columns.
  dplyr::select(Pheno,
                TraitEnv,Trait,Env,Year,Loc,Block,Col,Row,Plot,IDEU,Stage,Gen) |>
  droplevels()
...
ILSI_Pheno_w <- 
  ...
  # Simplify study name and convert it to a factor.
  mutate(study_name=as.factor(gsub('Urb_', '', study_name)),
         study_name=str_replace(study_name, '(\\w+)_(\\d+)', '\\2-\\1'),
  ) |>
  # Rename columns for easier reference.
  rename(
  ...
  mutate_at(vars(Env:Plot),as.factor) |>
  # Convert trait data to numeric format.
  mutate_at(vars(GY:HT), as.numeric) |>
  # Replace zero values with NA in trait columns.
  mutate_at(vars(GY:HT), ~ifelse(.<=0,NA,.)) |>
  mutate_at(vars(GY:HT), ~ifelse(Gen=='FILL',NA,.)) |>
  mutate(Gen = as.factor(ifelse(Gen=='FILL',NA,as.character(Gen)))) |>
  droplevels()

ILSI.MT_Pheno <- ILSI_Pheno_w |>
  pivot_longer(
    cols = GY:HT,  # Convert trait columns into a single column.
    names_to = 'Trait',  # Name of the new column for trait types.
    values_to = 'Pheno',  # Name of the column holding trait values.
    values_drop_na = FALSE  # Keep NA values in the data.
  ) |>
  ...
  ungroup() |>
  # Assign NA to outliers
  mutate(Pheno= ifelse(Trait=='GY' & Env=='23-SI' & Col=='33' & Row=='13', NA, Pheno)) |>
  mutate(
    IDEU = as.factor(paste(Env,Plot, sep='_'))
  ) |>
  # Create a new factor combining trait and environment.
  mutate(TraitEnv=as.factor(paste(Trait,Env,sep = '-'))) |>
  mutate(Stage=as.factor('SI')) |>
  # Select relevant columns.
  dplyr::select(Pheno,
                TraitEnv,Trait,Env,Year,Loc,Block,Col,Row,Plot,IDEU,Stage,Gen) |>
  droplevels()

ILSI_Pheno <- ILSI.MT_Pheno |>
  filter(Trait=='GY') |>
  droplevels()

IL.MT_Pheno <- bind_rows(ILYT_Pheno, ILSI.MT_Pheno)
str(IL.MT_Pheno)
IL_Pheno <- bind_rows(ILYT_Pheno, ILSI_Pheno)
str(IL_Pheno)
